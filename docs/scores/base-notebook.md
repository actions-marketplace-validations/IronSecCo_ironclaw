---
title: "base-notebook:latest container isolation score: 63/100 (grade C)"
description: "How isolated is base-notebook:latest by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# base-notebook:latest container isolation score: 63/100 (grade C)
...
Applying these to your `docker run base-notebook` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name base-notebook-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own base-notebook the same way this page was generated
ironctl scan my-base-notebook
```
...
Maintain **base notebook** (or run it)? Show its default-config isolation score with a badge that links back to this scorecard:

[![Container Isolation Score: 63/100 C](https://img.shields.io/badge/container%20isolation-63%2F100%20C-d4a72c)](https://ironsecco.github.io/ironclaw/scores/base-notebook/)
