---
title: "percona:8.0 container isolation score: 63/100 (grade C)"
description: "How isolated is percona:8.0 by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# percona:8.0 container isolation score: 63/100 (grade C)
...
> Graded from a read-only inspect of a **running container** started from `percona:8.0` at digest `sha256:bf3235cf8fe6c5e748c787eb8f55020fc2626cfb45859eec913777d97f5de9f2` with plain `docker run` defaults, its entrypoint overridden with `sleep` purely to keep it alive. The scan itself executes nothing inside the container. Scoring an image reference instead of a running container yields a different, non-comparable result. [How scoring works &rarr;](../scan.md)

## How it scores, dimension by dimension

| Dimension | Verdict | Score | What the scan found |
|-----------|:-------:|------:|---------------------|
| Non-root user (uid != 0) | ✅ PASS | 15/15 | runs as mysql (uid != 0) |
...
Applying these to your `docker run percona` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name percona-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own percona the same way this page was generated
ironctl scan my-percona
```
