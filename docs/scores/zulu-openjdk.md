---
title: "zulu-openjdk:21 container isolation score: 48/100 (grade D)"
description: "How isolated is zulu-openjdk:21 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# zulu-openjdk:21 container isolation score: 48/100 (grade D)

Run with plain `docker run azul/zulu-openjdk:21` defaults, no hardening flags, the **zulu openjdk** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.

> Graded from a read-only inspect of a **running container** started from `azul/zulu-openjdk:21` at digest `sha256:02d0098a502f664835eca406499de4d49da36d03a7b7e657b247716975c9cea9` with plain `docker run` defaults, its entrypoint overridden with `sleep` purely to keep it alive. The scan itself executes nothing inside the container. Scoring an image reference instead of a running container yields a different, non-comparable result. [How scoring works &rarr;](../scan.md)
...
Applying these to your `docker run zulu-openjdk` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name zulu-openjdk-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own zulu-openjdk the same way this page was generated
ironctl scan my-zulu-openjdk
```
...
Maintain **zulu openjdk** (or run it)? Show its default-config isolation score with a badge that links back to this scorecard:

[![Container Isolation Score: 48/100 D](https://img.shields.io/badge/container%20isolation-48%2F100%20D-e8873a)](https://ironsecco.github.io/ironclaw/scores/zulu-openjdk/)
