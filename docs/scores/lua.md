---
title: "lua:5.4 container isolation score: 48/100 (grade D)"
description: "How isolated is lua:5.4 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities, runs as root. Scan any container in 10s."
---

# lua:5.4 container isolation score: 48/100 (grade D)

Run with plain `docker run nickblah/lua:5.4` defaults, no hardening flags, the **lua** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.

> Graded from a read-only inspect of a **running container** started from `nickblah/lua:5.4` at digest `sha256:42e801f11377fce163a243e721d867b46f08f54efd4ff736ba1d969e4636a8ca` with plain `docker run` defaults, its entrypoint overridden with `sleep` purely to keep it alive. The scan itself executes nothing inside the container. Scoring an image reference instead of a running container yields a different, non-comparable result. [How scoring works &rarr;](../scan.md)
...
Applying these to your `docker run lua` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name lua-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own lua the same way this page was generated
ironctl scan my-lua
```
...
Maintain **lua** (or run it)? Show its default-config isolation score with a badge that links back to this scorecard:

[![Container Isolation Score: 48/100 D](https://img.shields.io/badge/container%20isolation-48%2F100%20D-e8873a)](https://ironsecco.github.io/ironclaw/scores/lua/)
