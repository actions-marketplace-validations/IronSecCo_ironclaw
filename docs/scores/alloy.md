---
title: "alloy:latest container isolation score: 48/100 (grade D)"
description: "How isolated is alloy:latest by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities, runs as root. Scan any container in 10s."
---

# alloy:latest container isolation score: 48/100 (grade D)

Run with plain `docker run grafana/alloy:latest` defaults, no hardening flags, the **alloy** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.

> Graded from a read-only inspect of a **running container** started from `grafana/alloy:latest` at digest `sha256:b8ec653c44235fbe910879145dac3597d66b0aaecf60bcbbe82580767771a839` with plain `docker run` defaults, its entrypoint overridden with `sleep` purely to keep it alive. The scan itself executes nothing inside the container. Scoring an image reference instead of a running container yields a different, non-comparable result. [How scoring works &rarr;](../scan.md)
...
Applying these to your `docker run alloy` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name alloy-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own alloy the same way this page was generated
ironctl scan my-alloy
```
