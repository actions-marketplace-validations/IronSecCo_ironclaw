---
title: "aspnet:9.0 container isolation score: 48/100 (grade D)"
description: "How isolated is aspnet:9.0 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities, runs as root. Scan any container in 10s."
---

# aspnet:9.0 container isolation score: 48/100 (grade D)
...
Applying these to your `docker run aspnet` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name aspnet-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own aspnet the same way this page was generated
ironctl scan my-aspnet
```
...
Maintain **aspnet** (or run it)? Show its default-config isolation score with a badge that links back to this scorecard:

[![Container Isolation Score: 48/100 D](https://img.shields.io/badge/container%20isolation-48%2F100%20D-e8873a)](https://ironsecco.github.io/ironclaw/scores/aspnet/)
