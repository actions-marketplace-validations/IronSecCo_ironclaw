---
title: "maven:3.9-eclipse-temurin-21 container isolation score: 48/100 (grade D)"
description: "How isolated is maven:3.9-eclipse-temurin-21 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in..."
---

# maven:3.9-eclipse-temurin-21 container isolation score: 48/100 (grade D)
...
Applying these to your `docker run maven` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name maven-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own maven the same way this page was generated
ironctl scan my-maven
```
